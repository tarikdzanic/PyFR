...
<%pyfr:kernel name='tfluxlinsplit1' ndim='2'
    ...
    for (int j = 0; j < ${nuvars}; j++) {
        F[j] = ${' + '.join(f'smats[1][{k}]*u[j][{k}]*f[j]' for k in range(ndims))};

        % if delta:
        F[j + ${nuvars}] = ${' + '.join(f'smats[1][{k}]*u[j][{k}]*f[j + {nuvars}]' for k in range(ndims))};
        % endif
    }
</%pyfr:kernel>
