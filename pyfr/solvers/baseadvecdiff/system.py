...
class BaseAdvectionDiffusionSystem(BaseAdvectionSystem):
    ...
    def _rhs_inv_graphs(self, uinbank, foutbank):
        ...
        g1.add_all(k['iint/comm_inv_flux'],
                   deps=k['eles/disu'] + k['mpiint/scal_fpts_pack'])
        g1.add_all(k['bcint/comm_inv_flux'],
        ...
        for l in k['mpiint/comm_inv_flux']:
        ...
        g2.add_all(k['eles/tdivtconf'], deps=k['mpiint/comm_inv_flux'])
    ...
    def _rhs_vis_graphs(self, uinbank, foutbank):
        ...
        # Compute the common normal flux at our internal/boundary interfaces
        g2.add_all(k['iint/comm_vis_flux'],
                   deps=ideps + k['mpiint/vect_fpts_pack'])
        g2.add_all(k['bcint/comm_vis_flux'], deps=ideps)
        ...
        for l in k['mpiint/comm_vis_flux']:
            ldeps = deps(l, 'mpiint/artvisc_fpts_unpack',
                         'mpiint/vect_fpts_unpack')
            g3.add(l, deps=ldeps)

        # Compute the transformed divergence of the corrected flux
        g3.add_all(k['eles/tdivtconf'], deps=k['mpiint/comm_vis_flux'])
